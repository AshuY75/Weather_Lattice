...
const weatherApi = axios.create({ baseURL: API_BASE_URL });
const airQualityApi = axios.create({ baseURL: AQI_BASE_URL });
const archiveApi = axios.create({ baseURL: ARCHIVE_BASE_URL });
...
export const fetchAirQualityData = async ({ latitude, longitude }) => {
  const params = {
    ...
    hourly: ['european_aqi', 'us_aqi', 'pm10', 'pm2_5', 'carbon_monoxide', 'nitrogen_dioxide', 'sulphur_dioxide', 'ozone', 'aerosol_optical_depth', 'dust', 'uv_index', 'uv_index_clear_sky', 'ammonia', 'alder_pollen', 'birch_pollen', 'grass_pollen', 'mugwort_pollen', 'olive_pollen', 'ragweed_pollen'],
    timezone: 'auto',
  };

  const response = await airQualityApi.get('/air-quality', { params });
  return response.data;
};
...
export const fetchHistoricalData = async ({ latitude, longitude, startDate, endDate }) => {
  ...
  const response = await archiveApi.get('/archive', { params });
  return response.data;
};
